...
var timeToggle = document.querySelector("#TimeSelect");
...
function saveTD(){
  ...
  update(ref(db, "Users/" + username + "/" + "Temporarily Information/"/*+ localStorage.clickcount*/), {
    CinemaName: localStorage.getItem("MovieName"),
    Time: localStorage.getItem("movieTime"),
    Place: localStorage.getItem("cinemaLocation"),
    Date: localStorage.getItem("MovieDate")
    // Place: Place
  })
  //Response of sucessful insertion
  .then(()=>{
    location = "seating_small.html"
  })
  //Response of error 
  .catch(()=> {
    alert(error)
...
timeToggle.addEventListener('click', saveTD);
